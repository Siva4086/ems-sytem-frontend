    ...
    <div className="min-h-screen flex flex-col md:flex-row">
        ...
        <div className="space-y-4">
            {portalOptions.map((portal) => (
                <Link key={portal.to} to={portal.to} className="group block bg-slate-50 border border-slate-200 rounded-lg p-5 sm:p-6 transition-all duration-300 hover:bg-indigo-50 hover:border-indigo-400">
                    {/* <portal.icon className="w-6 h-6 text-indigo-500 mr-4"/> */}
                    <div className="relative z-10 flex items-center justify-between gap-4 sm:gap-5">
                        <h3 className="text-lg text-slate-800 group-hover:text-indigo-600 transition-colors mb-1">{portal.title}</h3>
