...
const PrintPayslip = () => {
  ...
  useEffect(() => {   
       api.get(`/payslips/${id}`).then((res) => {
        setPayslip(res.data.data || res.data || null);
       }).catch(error => {
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-slate-50">
              ...
              <th className="text-right text-xs text-slate-500 uppercase tracking-wider px-4 py-3">Amount</th>
            </tr>
          </thead>  
          <tbody>
            <tr className="border-t border-slate-100">
              <td className="px-4 py-3 text-slate-700">Basic Salary</td>
              <td className="px-4 py-3 text-slate-900 font-medium text-right">${payslip.basicSalary?.toLocaleString()}</td>
            </tr>
            <tr className="border-t border-slate-100">
              <td className="px-4 py-3 text-slate-700">Allowances</td>
              <td className="px-4 py-3 text-slate-900 font-medium text-right">+${payslip.allowances?.toLocaleString()}</td>
            </tr>
            <tr className="border-t border-slate-100">
              <td className="px-4 py-3 text-slate-700">Deductions</td>
              <td className="px-4 py-3 text-slate-900 font-medium text-right">-${payslip.deductions?.toLocaleString()}</td>
            </tr>
             <tr className="border-t-2 border-slate-200 bg-slate-50">
              <td className="px-4 py-4 font-bold text-slate-900">Net Salary</td>
              <td className="px-4 py-4 text-slate-900 font-bold text-right text-lg">${payslip.netSalary?.toLocaleString()}</td>
            </tr>
          </tbody>
          </table>
      </div>
